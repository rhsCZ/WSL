...
// Compute the default swiotlb device-options token ("0x<base>,<size>M") sized to fit inside the
// VM's RAM. Returns an empty string for VMs too small to host the bounce buffer. Used when the
