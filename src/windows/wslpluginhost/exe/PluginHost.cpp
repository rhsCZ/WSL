...
HRESULT CALLBACK PluginHost::LocalWslcMountFolder(WSLCSessionId Session, LPCWSTR WindowsPath, BOOL ReadOnly, LPCWSTR Name, LPSTR Mountpoint)
{
    auto* host = g_pluginHost.load(std::memory_order_acquire);
    if (host == nullptr || host->m_callback == nullptr)
    {
    ...
    // Zero-init the plugin's buffer before the cross-process call so the
    // plugin observes a defined string even if the service-side callback
    // fails before writing anything.
    ZeroMemory(Mountpoint, WSLC_MOUNTPOINT_LENGTH);

    ScopedComInitForCallback coInit;
    RETURN_IF_FAILED(coInit.Result());

    return host->m_callback->WslcMountFolder(Session, WindowsPath, ReadOnly, Name, Mountpoint);
}
