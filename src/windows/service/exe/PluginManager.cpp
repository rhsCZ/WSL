...
STDMETHODIMP PluginHostCallbackImpl::WslcMountFolder(
    _In_ DWORD SessionId, _In_ LPCWSTR WindowsPath, _In_ BOOL ReadOnly, _In_ LPCWSTR Name, _Out_writes_(WSLC_MOUNTPOINT_LENGTH) CHAR* Mountpoint)
try
{
    RETURN_HR_IF(E_POINTER, WindowsPath == nullptr || Name == nullptr || Mountpoint == nullptr);
    Mountpoint[0] = '\0';

    const auto nameLength = wcslen(Name);
    RETURN_HR_IF_MSG(
        E_INVALIDARG,
        nameLength == 0 ||
    ...
    if (SUCCEEDED(result))
    {
        RETURN_HR_IF(E_UNEXPECTED, strcpy_s(Mountpoint, WSLC_MOUNTPOINT_LENGTH, linuxPath.c_str()) != 0);
    }
